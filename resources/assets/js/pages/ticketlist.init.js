...
function SearchData() {
    ...
    ticketsList.filter(function (e) {
        var t = (matchData = new DOMParser().parseFromString(
            ...
            a = "all" == t || "all" == s || t == s,
            i =
                new Date(e.values().create_date.slice(0, 12)) >= new Date(l) &&
                new Date(e.values().create_date.slice(0, 12)) <= new Date(d);
...
var count = 14,
    example =
        ...
        editBtn.addEventListener("click", function (e) {
            document.getElementById("exampleModalLabel").innerHTML =
                "Edit Order";
            var t = ticketsList.get({ id: idField.value });
            ...
            Array.from(e).forEach(function (e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId &&
                    document
                        ...
                        .addEventListener("click", function () {
                            ticketsList.remove("id", t.outerHTML),
                                document
        ...
        ? Swal.fire({
              ...
              if (e.value) {
                  for (i = 0; i < ids_array.length; i++)
                      ticketsList.remove(
                          "id",
                          `<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="${ids_array[i]}" class="fw-medium link-primary ticket-id">#VLZ${ids_array[i]}</a>`
                      );
...
function ViewTickets(e) {
    ...
    localStorage.setItem("ticket-list", JSON.stringify(e[0]._values)),
        localStorage.setItem("option", "view-ticket"),
        localStorage.setItem("ticket_no", t),
